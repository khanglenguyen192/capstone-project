...
import { Row, Col, Input, message, Upload, Popover } from "antd";
...
export default function DepartmentPage(props) {
  ...
  const handleClickDepartment = (event, param) => {
    DepartmentService.getChildren(param, user.token).then((res) => {
      const response = res.data;
  ...
  return (
    <div
      ...
      style={{ backgroundColor: Colors.white_content }}
    >
      ...
      <Row gutter={[50, 60]} style={{ margin: "0 20px" }}>
        {departmentList.map((item) => (
          <Popover content={content} placement="top">
            <Col
              span={6}
              onClick={(event) => handleClickDepartment(event, item.id)}
            >
